...
function fillOriginalVsQuantized(result) {
  ...
  // CLEANUP
  //container.innerHTML = "";
  $$('imageView2-statistics').getNode().firstElementChild.innerHTML =
    ' (SSIM: ' + result.ssim.toFixed(2) + ', Time: ' + result.time + ' )';

  // DRAW ORIGINAL IMAGE
  var canvas = QuantizationUsage.drawPixels(
    result.original,
    result.original.getWidth(),
  );
  ...
  // DRAW PALETTE
  canvas = QuantizationUsage.drawPixels(
    result.palette.getPointContainer(),
    16,
    128,
  );
...
function fillClickToCompare(result) {
  var prefix = 'id-imageView2-';
  // CLEANUP
  $$('imageView1-statistics').getNode().firstElementChild.innerHTML =
    ' (SSIM: ' + result.ssim.toFixed(2) + ', Time: ' + result.time + ' )';

  // DRAW ORIGINAL IMAGE
  var canvasOriginal = QuantizationUsage.drawPixels(
    result.original,
    result.original.getWidth(),
  );
  ...
  // DRAW REDUCED/DITHERED IMAGE
  var canvasReduced = QuantizationUsage.drawPixels(
    result.image,
  ...
  // DRAW PALETTE
  var canvasPalette = QuantizationUsage.drawPixels(
    result.palette.getPointContainer(),
    16,
    128,
  );
