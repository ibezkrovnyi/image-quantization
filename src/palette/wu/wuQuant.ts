...
module IQ.Palette {
	...
	function createArray4D(dimension1, dimension2, dimension3, dimension4) : number[][][][] {
	...
	function createArray3D(dimension1, dimension2, dimension3) : number[][][] {
		var a = new Array(dimension1);
		for (var i = 0; i < dimension1; i++) {
			a[i] = new Array(dimension2);
			for (var j = 0; j < dimension2; j++) {
	...
	function fillArray3D<T>(a : T[][][], dimension1, dimension2, dimension3, value : T) : void {
		for (var i = 0; i < dimension1; i++) {
			a[i] = [];
			for (var j = 0; j < dimension2; j++) {
				a[i][j] = [];
	...
	function fillArray1D<T>(a : T[], dimension1, value : T) : void {
		for (var i = 0; i < dimension1; i++) {
			a[i] = value;
		}
	}
	...
	export class WuQuant {
		...
		public quantize() : Utils.Palette {
			...
			var palette : Utils.Palette = new Utils.Palette();
			...
			for (var paletteIndex = 0; paletteIndex < this._colors; paletteIndex++) {
				if (this._sums[paletteIndex] > 0) {
					...
					var color = Utils.Point.createByRGBA(r | 0, g | 0, b | 0, a | 0);
					palette.add(color);
				}
			}
		...
		private _preparePalette() : void {
			...
			for (var index = 0, l = this._pixels.length; index < l; index++) {
				var color : Utils.Point = this._pixels[index];
		...
		private _addColor(color : Utils.Point) : void {
